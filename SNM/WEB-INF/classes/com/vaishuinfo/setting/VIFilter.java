...
import com.vaishuinfo.managedbean.LoginBean;
...
public class VIFilter implements Filter {
    ...
    private void doBeforeProcessing(RequestWrapper request, ResponseWrapper response)
        ...
        if (debug) {
            log("P41Filter:DoBeforeProcessing");
        }
    ...
    private void doAfterProcessing(RequestWrapper request, ResponseWrapper response)
            throws IOException, ServletException {
        if (debug) {
            log("P41Filter:DoAfterProcessing");
        }
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        String pageRequested = wrappedRequest.getRequestURL().toString();
        ...
        try {
            ...
            if (pageRequested.contains("javax.faces.resource")) {
                chain.doFilter(wrappedRequest, wrappedResponse);
            } else {
                Object ob = session.getAttribute("loginBean");
                if (ob
                        == null && !pageRequested.contains(
                        "login.xhtml")) {
                    wrappedResponse.sendRedirect("login.xhtml?faces-redirect=true");
                } else { // Login Bean created means check the access
                    if (ob == null && pageRequested.contains(
                            "login.xhtml")) {
                        chain.doFilter(wrappedRequest, wrappedResponse);
                    } else {
                        LoginBean login = (LoginBean) ob;
                        if (login.getLogDTO().isIsLoggedIn()) {
                            chain.doFilter(wrappedRequest, wrappedResponse);
                        } else if (!pageRequested.contains(
                                "login.xhtml")) {
                            wrappedResponse.sendRedirect("login.xhtml?faces-redirect=true");
                        } else {
                            chain.doFilter(wrappedRequest, wrappedResponse);
    ...
    public void init(FilterConfig filterConfig) {
        ...
        if (filterConfig != null) {
            if (debug) {
                log("P41Filter: Initializing filter");
            }
        }
    }
    ...
    public String toString() {
        ...
        StringBuffer sb = new StringBuffer("P41Filter(");
    ...
    class RequestWrapper extends HttpServletRequestWrapper {
        ...
        public void setParameter(String name, String[] values) {
            if (debug) {
                System.out.println("P41Filter::setParameter(" + name + "=" + values + ")" + " localParams = " + localParams);
            }
        ...
        public String getParameter(String name) {
            if (debug) {
                System.out.println("P41Filter::getParameter(" + name + ") localParams = " + localParams);
            }
            if (localParams == null) {
        ...
        public String[] getParameterValues(String name) {
            if (debug) {
                System.out.println("P41Filter::getParameterValues(" + name + ") localParams = " + localParams);
            }
            if (localParams == null) {
        ...
        public Enumeration getParameterNames() {
            if (debug) {
                System.out.println("P41Filter::getParameterNames() localParams = " + localParams);
            }
            if (localParams == null) {
        ...
        public Map getParameterMap() {
            if (debug) {
                System.out.println("P41Filter::getParameterMap() localParams = " + localParams);
            }
            if (localParams == null) {
